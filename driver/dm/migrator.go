...
func (m Migrator) MigrateColumn(dst interface{}, field *schema.Field, columnType gorm.ColumnType) error {
	...
	//字段定义longtext，数据库会自动变成text，但不会报错。
	//迁移库时查询出来是text，定义longtext，导致属性修改问题（add by lguo）
	if strings.ToLower(fullDataType) == "longtext" {
		fullDataType = "text"
	}
	...
	if !isSameType {
		...
		if length, ok := columnType.Length(); length != int64(field.Size) {
			if columnType.DatabaseTypeName() == "VARCHAR" {
				//字符串类型的字段，SQL查询的长度是定义的2倍，对比后会导致执行alter，所以此处除以2。
				//例如：定义的是varchar(20)，SQL查询后相应的是40（add by lguo）。
				length = length / 2
			}
